...
import { 
  ...
  HeartHandshake,
  FileQuestion,
  Menu, // Import the Menu icon for mobile toggle
} from 'lucide-react'
...
export const LoveJournalSidebar: React.FC<LoveJournalSidebarProps> = ({ session }) => {
  ...
  const { collapsed, setCollapsed } = useSidebar(); // Use the sidebar context
  ...
  useEffect(() => {
    ...
    // Check for mobile device on initial load and on resize
              ...
              {partnerConnected ? (
                ...
                >
                  <Avatar className="h-8 w-8 ring-2 ring-rose-500 ring-offset-2">
                    ...
                    <AvatarFallback className="bg-gradient-to-br from-fuchsia-400 to-purple-500 text-white">{getInitials(session?.user?.name ?? "") || "IN"}</AvatarFallback>
                  </Avatar>
                  <motion.div 
